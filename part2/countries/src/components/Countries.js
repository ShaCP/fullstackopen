...
const baseWeatherApiUrl = (lat, lon) => `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${process.env.REACT_APP_API_KEY}&units=metric`;

const Country = ({
    country: { name: { common: name }, capital, population, languages, flags, latlng: [lat, lon] },
    isOnlyResult,
}) => {
...
export const Countries = ({ countries, filterFor }) => {
    const filteredCountries = filterFor
        ? countries.filter((country) =>
            country.name.common.toLowerCase().includes(filterFor.toLowerCase())
        )
        : [];
    ...
    return (!!filteredCountries.length && (<div className="countries">
